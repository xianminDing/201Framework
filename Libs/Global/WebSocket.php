 ...
 * WebSocket类
 * by zhenbianshu
 * github: https://github.com/zhenbianshu/websocket
 * 2018年09月10日
 */
class Libs_Global_WebSocket {
    const LOG_PATH = '/tmp/';
    const LISTEN_SOCKET_NUM = 20000;
    ...
    private function debug(array $info) {
        ...
        file_put_contents(self::LOG_PATH . 'websocket_debug'.date('Ymd').'.log', implode(' | ', $info) . "\r\n", FILE_APPEND);
    }
    ...
    private function error(array $info) {
        ...
        $info = array_map('json_encode', $info);
        file_put_contents(self::LOG_PATH . 'websocket_error'.date('Ymd').'.log', implode(' | ', $info) . "\r\n", FILE_APPEND);
    }
}